...
- `bulkUploadStyles` (POST, `{items: [{name, html, description?, tags?, eventTypes?, designNotes?, adminRating?}, ...]}`) — batch insert up to 50 style library items, optional rating applied on upload
- `bulkRateStyles` (POST, `{ratings: [{styleId, rating, notes?}, ...]}`) — rate multiple styles at once
- `bulkDeleteStyles` (POST, `{styleIds: [id, ...]}`) — permanently delete up to 100 style library items
